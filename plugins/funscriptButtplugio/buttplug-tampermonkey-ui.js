  ...
  <div id="buttplug-dialog">
        ...
        <div id="buttplug-container">
          <div id="buttplug-connector">
            <a class="buttplug-button" id="buttplug-connect-browser">Connect in Browser</a>
            <br/>
            <a class="buttplug-button" id="buttplug-connect-intiface">Connect to Intiface Desktop</a>
            <br/>
          </div>
          <div id="buttplug-enumeration">
            <a class="buttplug-button" id="buttplug-scanning">Start Scanning</a>
            <a class="buttplug-button" id="buttplug-disconnect">Disconnect</a>
            <br/>
            <h3>Devices</h3>
    ...
    setTimeout(
      () =>
        (function () {
          // Set up Buttplug
          const buttplug_client = new buttplug.ButtplugClient("Stash Client");
          const connector_div = document.getElementById("buttplug-connector");
          const enumeration_div = document.getElementById(
            "buttplug-enumeration"
          );
          const scanning_button = document.getElementById("buttplug-scanning");
          const connect_browser_button = document.getElementById(
            "buttplug-connect-browser"
          );
          const connect_intiface_button = document.getElementById(
            "buttplug-connect-intiface"
          );
          const disconnect_button = document.getElementById(
            "buttplug-disconnect"
          );
          ...
          connect_browser_button.addEventListener(
            "click",
            async (event) => {
          ...
          connect_intiface_button.addEventListener(
            "click",
            async (event) => {
              const connector =
                new buttplug.ButtplugBrowserWebsocketClientConnector(
                  "wss://intiface.gustavsson.tech"
                );
              await buttplug_client.connect(connector);
              connector_div.style.display = "none";
              enumeration_div.style.display = "block";
          ...
          disconnect_button.addEventListener(
            "click",
            async (event) => {
